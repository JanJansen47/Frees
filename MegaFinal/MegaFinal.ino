...
/*
 ...
 Connections schema Teensy
 Naar "Smartcontroller"
 
 Teensy    name      UNO    name
 7        RX3        1      TX
 8        TX3        0      RX
 GND      GND       GND     GND
...
#ifdef MEGAmarc
LiquidCrystal lcd(44, 48, 43, 45, 47, 49); //wiring Marc (special board gemaakt door Marc)
#endif
#ifdef MEGAjan
LiquidCrystal lcd(22, 23, 25, 27, 29, 31); //wiring Jan (Provisories door jan)
#endif
#ifdef Teensy
LiquidCrystal lcd(23, 22, 21, 20, 19, 18); //wiring Teensy
#endif
...
uint8_t select_sc2;
#ifdef MEGAmarc
const int buttonPin = 39;  // for test only wiring Marc
#endif
#ifdef MEGAjan
const int buttonPin = 37;  // for test only wiring Jan
#endif
#ifdef Teensy
const int buttonPin = 2;  // for test only wiring Jan
#endif
...
//M2tk m2(&opstart, m2_es_arduino, m2_eh_4bs, m2_gh_arduino_serial);
...
M2_LABEL(opstart_3, "x10y2" , "Mega_Final");
...
//M2_LABELFN(run_label1, "x12y0", fn_run);
//M2_LABELFN(run_label3, "x12y0", fn_run2);
...
void setup() {
 ...
 m2.setPin(M2_KEY_ROT_ENC_B, 41);  //Wiring Marc
  ...
  #endif
   #ifdef Teensy
  m2.setPin(M2_KEY_ROT_ENC_A, 0); //Wiring Jan
  m2.setPin(M2_KEY_ROT_ENC_B, 1);  //Wiring Jan
  #endif
}
...
void fn_service_s(m2_el_fnarg_p fnarg) {   // Service mode
  ...
  Serial3.write('CTRL-X'); // Reset Grbl
   XX.sendsyscom("$X");   // reset Grbl  // new 21-08-2015
...
void fn_run2(m2_el_fnarg_p fnarg) {
}
...
void fn_run(m2_el_fnarg_p fnarg) {
  ...
  errorhandling (error);
  error = home();    
  error = XX.sendgcode ("G38.2 Z-40 F200");  // probe
  error = XX.sendgcode("G91"); // relative postion mode
  error = XX.sendgcode("G1Z20F20000"); // probe 20 mm up
  error = XX.sendgcode("G90");  // absolute position mode
  error = XX.sendgcode("G10L20P1X0Y0Z0"); // Set G54 to homing origin
  error = XX.sendgcode("G54"); // work with G54 origin
  error = XX.sendgcode("M3");   // Spindle enable
  error = XX.sendgcode(Start_spindel_speed);   // Spindle at 0 rev/min  // foutje!!
  byte returndir = 1;
...
Stop = HIGH;
   ...
   error = XX.sendgcode("G54");  // om eventueel puls verlies te testen
    error = XX.sendgcode("G1x0.0Y0.0Z0.0");  // om eventueel puls verlies te testen
...
const char *fs_strlist_getstr(uint8_t idx, uint8_t msg)  {
   Serial.println(" print de filelist)");
  // for (int l=0; l<3 ;l++) { Serial.print (YY.filelist[l]); Serial.println();}  // test 22-08-2015
